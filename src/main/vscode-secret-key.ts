...
function getSafeStorage(): { isEncryptionAvailable(): boolean; encryptString(s: string): Buffer; decryptString(b: Buffer): string } {
    ...
    try {
      ...
      if (safeStorage.isEncryptionAvailable()) {
        ...
        if (cachedKey.length === KEY_LENGTH) {
          console.log(`[vscode-secret-key] loaded existing key (${KEY_LENGTH} bytes) from ${KEY_PATH}`)
          return cachedKey
        }
      ...
      console.warn('[vscode-secret-key] failed to read key file, regenerating.', 
        'This can happen when switching between dev and packaged mode (safeStorage uses different Keychain entries).', err)
      cachedKey = null
    }
  }
