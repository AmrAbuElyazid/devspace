...
type SidebarDropTarget =
  | { type: 'sidebar-folder'; folderId: string; container: SidebarContainer; parentFolderId: string | null }
  | { type: 'sidebar-workspace'; workspaceId: string; container: SidebarContainer; parentFolderId: string | null }
...
export function resolveSidebarDrop(input: ResolveSidebarDropInput): SidebarDropResolution | null {
  ...
  if (over.type === 'sidebar-folder') {
    const rect = input.rects?.[`folder-${over.folderId}`] ?? input.rects?.[`folder-folder-${over.folderId}`]
    if (!rect) return null
  ...
  const rect = input.rects?.[`ws-${over.workspaceId}`] ?? input.rects?.[`workspace-${over.workspaceId}`]
  if (!rect) return null
