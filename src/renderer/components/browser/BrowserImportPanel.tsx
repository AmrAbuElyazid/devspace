  ...
  useEffect(() => {
    ...
    void window.api.browser
      ...
      .catch((error) => {
        ...
        setProfilesMessage(
          error instanceof Error ? error.message : "Failed to load Chrome profiles.",
        );
      });
  ...
  async function handleImport(mode: BrowserImportMode): Promise<void> {
    ...
    try {
      ...
      if (result.ok) {
        setImportState({
          status: "success",
          message: buildSuccessMessage(browser, mode, result, selectedChromeProfileName),
        });
        return;
      }
        ...
        <div className="browser-import-card-header">
          <div>
            ...
            <p className="browser-import-card-copy">
              Import existing browsing data into the in-app browser session.
            </p>
          </div>
        </div>
...
function buildSuccessMessage(
  ...
  const source =
    browser === "chrome" && selectedChromeProfileName
      ? `Chrome (${selectedChromeProfileName})`
