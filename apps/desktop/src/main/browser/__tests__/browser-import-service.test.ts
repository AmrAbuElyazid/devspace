...
test("listChromeProfiles reads available Chrome profile directories", async () => {
  ...
  try {
    ...
    const service = new BrowserImportService({
      ...
      sessionManager: {
        getSession: () =>
          ({ cookies: { set: async () => undefined, flushStore: async () => undefined } }) as never,
      },
...
test("importChrome returns retryable keychain failure after importing history", async () => {
  ...
  expect(importedHistory).toEqual([
    {
      url: "https://example.com",
      title: "Example",
      visitedAt: 1,
...
test("importChrome accepts very large Chrome history timestamps from sqlite bigint rows", async () => {
  ...
  expect(importedHistory[0]?.url).toBe("https://example.com/large-time");
  expect(Number.isFinite(importedHistory[0]?.visitedAt)).toBeTruthy();
});
...
test("importChrome supports history-only imports without loading cookies", async () => {
  ...
  expect(loadChromeCookiesCalls).toBe(0);
  expect(importedHistory.length).toBe(1);
  expect(result).toEqual({ ok: true, importedCookies: 0, importedHistory: 1 });
});
