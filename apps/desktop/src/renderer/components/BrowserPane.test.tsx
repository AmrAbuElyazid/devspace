...
test("opens link context-menu targets in a new tab for the focused group", async () => {
  const contextMenuShow = vi.mocked(window.api.contextMenu.show);
  contextMenuShow.mockResolvedValueOnce("link-open-new-tab");
  ...
  expect(browserPaneMocks.contextMenuRequestHandler).toBeTypeOf("function");
  const registeredHandler = browserPaneMocks.contextMenuRequestHandler;
  if (!registeredHandler) {
    throw new Error("expected BrowserPane to register a context menu handler");
  }
  ...
  expect(contextMenuShow).toHaveBeenCalledWith(
    [
  ...
  expect(browserPaneMocks.openBrowserInGroup).toHaveBeenCalledWith(
    "workspace-1",
    "group-1",
