 ...
 *
 * Architecture (adapted from cmux's KeyboardShortcutSettings):
 ...
 * - DEFAULT_SHORTCUTS: the complete map, derived from cmux's shortcut set
 *
...
// ---------------------------------------------------------------------------
// Default shortcuts — derived from cmux's shortcut set
// ---------------------------------------------------------------------------
...
export const DEFAULT_SHORTCUTS: readonly ShortcutDefinition[] = [
  ...
  // Cmd+1-9 = select workspace by number (cmux convention)
  ...
  // Ctrl+1-9 = select tab by number (cmux convention)
  ...([1, 2, 3, 4, 5, 6, 7, 8, 9] as const).map(
    (n): ShortcutDefinition => ({
