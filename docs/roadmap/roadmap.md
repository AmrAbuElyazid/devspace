...
- [ ] Continue breaking up large files where it meaningfully improves maintenance. `PaneGroupContainer.tsx` plus browser-pane-manager's view creation, raw `webContents` actions, runtime state transitions, and repeated pane lookup boilerplate have been split into smaller pieces, and `styles/index.css` now delegates workspace-shell, browser-pane, and shared UI primitive sections to dedicated files; remaining initial targets: the rest of `apps/desktop/src/main/browser/browser-pane-manager.ts` and the rest of `apps/desktop/src/renderer/styles/index.css`
