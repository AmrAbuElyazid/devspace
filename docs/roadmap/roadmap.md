...
- [ ] Continue breaking up large files where it meaningfully improves maintenance. `PaneGroupContainer.tsx` plus browser-pane-manager's view creation, raw `webContents` actions, and runtime state transitions have been split into smaller pieces; remaining initial targets: the rest of `apps/desktop/src/main/browser/browser-pane-manager.ts` and `apps/desktop/src/renderer/styles/index.css`
