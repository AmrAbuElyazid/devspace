...
- [ ] Clarify the current package and desktop distribution posture in repo docs. `ghostty-electron` is intended as a public OSS package, but it still exports raw TypeScript and documents missing publish flow; Devspace desktop now has a signed/notarized macOS-arm64 DMG flow, but CI publishing and broader distribution posture still need clearer documentation. Refs: `packages/ghostty-electron/package.json`, `packages/ghostty-electron/README.md`, `apps/desktop/package.json`, `docs/release-process.md`
...
- [ ] Reduce test-environment coupling in the desktop suite. The current test baseline is broad, but some coverage still depends on real loopback listeners and a few suites still emit warning noise during otherwise-green runs. Split `cli-server` coverage into mostly pure handler tests plus a smaller real-network smoke path, and fix the remaining React key warning in the note-editor slash menu tests so local and sandboxed runs stay predictable. Refs: `apps/desktop/src/main/cli-server.ts`, `apps/desktop/src/main/cli-server.test.ts`, `packages/note-editor/src/plate-ui/slash-node.tsx`, `packages/note-editor/src/plate-ui/slash-node.test.tsx`
...
- [ ] Continue the remaining performance hardening pass from `docs/audits/performance.md`. The remaining work is mostly turning the new instrumentation and retention stress passes into useful baselines, then revisiting active-workspace hidden-terminal retention if real memory pressure still shows up, any native-view churn that still appears in profiling, and terminal event batching only if bursty workloads still justify it. Refs: `docs/audits/performance.md`, `apps/desktop/src/main/performance-monitor.ts`, `apps/desktop/e2e/hidden-terminal-retention.spec.ts`, `packages/ghostty-electron/native/ghostty_bridge.mm`, `packages/ghostty-electron/src/terminal-manager.ts`, `apps/desktop/src/main/browser/browser-pane-manager.ts`, `apps/desktop/src/renderer/store/native-view-store.ts`, `apps/desktop/src/renderer/hooks/useDndOrchestrator.ts`
