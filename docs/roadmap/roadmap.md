...
- [ ] Continue the remaining performance hardening pass from `docs/audits/performance.md`. The remaining work is mostly measurement and any native-view churn that still shows up in profiling, plus the still-eager settings text/number persistence paths and drag invalidation if profiling justifies deeper changes. Refs: `docs/audits/performance.md`, `packages/ghostty-electron/native/ghostty_bridge.mm`, `packages/ghostty-electron/src/terminal-manager.ts`, `apps/desktop/src/main/browser/browser-pane-manager.ts`, `apps/desktop/src/renderer/store/native-view-store.ts`, `apps/desktop/src/renderer/store/workspace-sidebar-metadata.ts`, `apps/desktop/src/renderer/hooks/useDndOrchestrator.ts`
