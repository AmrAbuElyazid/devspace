...
- [ ] Continue breaking up large files where it meaningfully improves maintenance. `PaneGroupContainer.tsx` plus browser-pane-manager's view creation, pane registry bookkeeping, runtime snapshots, repeated pane lookup/state-change dispatch flow, runtime patch/failure reporting, browser history tracking, permission tracking, webContents listener bindings, raw `webContents` actions, and runtime state transitions have been split into smaller pieces, and renderer styles are now split across dedicated workspace-shell, browser-pane, UI-primitives, base, and design-token files; remaining initial targets: the rest of `apps/desktop/src/main/browser/browser-pane-manager.ts`
