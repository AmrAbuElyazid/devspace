...
- [ ] Expand behavior-heavy UI and integration coverage where it pays off most.
      ...
      Progress: added direct lifecycle tests for `useNativeView` and `TerminalPane`, `NoteEditor` package serialization tests, `BrowserPane` permission/security plus context-menu routing coverage, and `main/index.ts` startup/window shutdown coverage.
