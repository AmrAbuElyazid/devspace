...
- [ ] Expand behavior-heavy UI and integration coverage where it pays off most.
      ...
      Progress: added direct lifecycle tests for `useNativeView` and `TerminalPane`, `NoteEditor` package serialization tests, `BrowserPane` permission/security plus context-menu routing coverage, `main/index.ts` startup/window shutdown coverage, drag/drop tests around `useDndOrchestrator` target priority, `PaneGroupContainer` native-view placeholders, the core handler matrix for tab reorder/split plus workspace/sidebar routing, and `Sidebar` shell coverage for search, pending edits, workspace/folder creation routing, context-menu actions, and pinned drag state.
